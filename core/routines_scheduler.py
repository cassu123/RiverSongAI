...
async def _check_routines(app: FastAPI):
    ...
    for r in routines:
        ...
        if r["trigger"] == "time" and r["time"] == now_local.strftime("%H:%M"):
            # Check if it ran recently to avoid double-triggers in the same minute
            ...
            if last_run:
                try:
                    ...
                    # Don't skip — let it run, which will write a fresh valid timestamp

            logger.info("Triggering scheduled routine '%s' for user %s", r["name"], user_id)
...
async def _run_proactive_routine(app: FastAPI, user_id: str, routine: dict):
    ...
    if not active_sockets:
        logger.info("No active WebSocket for user %s. Routine will run silently (save to history).", user_id)
    ...
    try:
        ...
        await loop.run_text(routine["prompt"], broadcast)
