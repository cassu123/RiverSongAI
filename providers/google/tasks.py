...
class GoogleTasksProvider:
    ...
    def _get_service(self):
        if self._service is None:
            ...
            self._service = self._auth.build_service(
                self._user_id, "tasks", "v1")
    ...
    async def get_tasks(self, tasklist_id: str = "@default",
    ...
    async def create_task(
            self, title: str, notes: Optional[str] = None, tasklist_id: str = "@default") -> Dict[str, Any]:
        """Create a new task."""
        def _sync():
            service = self._get_service()
            task = {
