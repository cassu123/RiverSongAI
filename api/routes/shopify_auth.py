...
def _get_provider(request: Request):
    ...
    if not settings.shopify_api_key or not settings.shopify_api_secret:
        raise HTTPException(status_code=500, detail="Shopify API keys not configured.")
...
async def shopify_auth_callback(
    ...
    if not provider.verify_hmac(params):
        logger.warning("Shopify auth callback HMAC verification failed for shop %s", shop)
    ...
    try:
        ...
        logger.info("Successfully connected Shopify store %s for user %s", shop, user_id)
