...
@router.post("/zones")
async def create_zone(body: ZoneBody, user: dict = Depends(require_role("operator", "admin"))):
    user_id = user["sub"]
    now = datetime.now(timezone.utc).isoformat()
    store = SQLiteStore()
    zone_id = uuid.uuid4().hex
...
async def run_program(id: str, user: dict = Depends(require_role("operator", "admin"))):
    user_id = user["sub"]
    store = SQLiteStore()
    prog = await store.execute_read_one_async("SELECT * FROM vector_programs WHERE program_id=?", (id,))
