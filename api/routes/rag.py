...
async def query_rag(
    ...
    try:
        ...
        if not results:
            return {"answer": "I couldn't find any information in the uploaded documents.", "chunks": []}
        ...
        await loop.run_text(prompt, collect)
        
        return {
            "answer": "".join(answer_parts),
            "chunks": [
                {"text": r["text"], "source": r["metadata"].get("filename", "Unknown")}
                for r in results
            ]
        }
    except Exception as e:
...
from fastapi import Header # Fix missing import
